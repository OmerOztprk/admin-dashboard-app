...
export class AuthService {
    ...
    changePassword(currentPassword: string, newPassword: string): Observable<User> {
        ...
        return this.http.post<User>(`${this.API_URL}/users/change-password`, passwordData)
            .pipe(
                tap(() => {
                    // Şifre değiştikten sonra yapılacak işlemler
                }),
                catchError(error => {
                    console.error('Password change error:', error);
                    return throwError(() => new Error(error.error?.message || 'Şifre değiştirme başarısız oldu'));
                })
            );
    }
