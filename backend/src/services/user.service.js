...
exports.add = async (body, currentUser) => {
  if (!body.email || !body.password || !Array.isArray(body.roles) || body.roles.length === 0) {
    throw new CustomError(400, "Validation error", "Email, password ve roller zorunludur.");
  }
...
exports.update = async (body, currentUser) => {
  if (!body._id) throw new CustomError(400, "Validation error", "_id alanı zorunludur.");
  ...
  AuditLogs.info(currentUser.email, "Users", "Update", { _id: body._id, updates });
