...
exports.add = async (req, res) => {
  try {
    ...
    const customer = await Customer.create({ name, slug, email, phone });
...
exports.update = async (req, res) => {
  try {
    const { _id, name, slug, email, phone, is_active } = req.body;
    const updated = await Customer.findByIdAndUpdate(
      _id,
      { name, slug, email, phone, is_active, updated_at: new Date() },
      { new: true }
    );
